...
def max_pool_forward_naive(x, pool_param):
  ...
  #############################################################################
  pass
  # get number of data points N, number of channels C, height H, and width W
  N, C, H, W = x.shape
  # get pooling layer parameters
  pool_height = pool_param['pool_height']
  pool_width = pool_param['pool_width']
  stride = pool_param['stride']


  # get pooling layer output heigth and width
  ...
  for n in xrange(N):
  	...
  	for c in xrange(C):
  		# iterating over heigth of input
  		for h_output in xrange(H_output):
  			# iterating over width of input
  			for w_output in xrange(W_output):
  				# first row of max window
  				h1 = h_output * stride
  				# last row of max window
  				h2 = h1 + pool_height
  				# first column of max window
  				w1 = w_output * stride
  				# last column of max window
  				w2 = w1 + pool_width
  				# max pooling w
  				pool_grid = x[n,c,h1:h2, w1:w2]
  				# output of max pooling layer
  				out[n,c,h_output,w_output] = np.max(pool_grid)
  #############################################################################
  #                             END OF YOUR CODE                              #
  #############################################################################
  cache = (x, pool_param)
  	...
  	for c in xrange(C):
  		...
  		for h_output in xrange(H_output):
  			...
  			for w_output in xrange(W_output):
  				...
  				# Gradient of max with respect to x
  				# local gradient is 1 for max value and zero otherwise
  				dx[n, c, h1:h2, w1:w2]= np.reshape(window3,(pool_height,W_output))* dout[n, c, H_output, w_output]
  pass
  #############################################################################
  #                             END OF YOUR CODE                              #
  #############################################################################
