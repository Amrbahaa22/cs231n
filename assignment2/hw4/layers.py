...
# normalizing Xhat: find distaance of x and sample_mean over(step3,4 and 5 ---step5=  eps for numeric stability and take root from them)
        Xhat = (x - sample_mean) / np.sqrt(sample_var + eps)
#scaling and shifting
#output of bactchnorm forward
        out = gamma * Xhat + beta
...
#unpacking the variable of Fw pass
    ...
    N,D=x.shape
#calculate the gradients of input by chain rule, multiplying local gradients gradient of output
...
#multiply local gradient over  with gradient flowing
  ...
  # gradient from above
    grad_running_var=np.sum(np.multiply(dXhat,x-running_mean),axis=0)*(-0.5)*(dvarX)**(-3.0/2.0)

    d_running_mean=np.sum(dXhat*(-1.0/np.sqrt(dvarX) ) ,axis=0)+grad_running_var*(np.sum(-2*(x-running_mean),axis=0)/N)
...
#gradient with the respect of gamma
    dgamma=np.sum(dout*Xhat,axis=0)
#gradient withe the respect of Beta
    dbeta=np.sum(dout,axis=0)
